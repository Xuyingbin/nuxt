        ...
        <div class="container">
            <div class="theme" v-if="themeDetailBasicInfo">
                ...
                <div class="follow">
                    ...
                    <span v-if="themeDetailBasicInfo.attention"
                        @click="cancelFollow({id: themeDetailBasicInfo.id})"
                    ><b>取消关注</b></span>
                </div>
            </div>
        </div>
...
import themeDetailsChart from "~/components/themeDetails/ThemeDetailsChart";
import pieChart from "~/components/themeDetails/PieChart";
import RelatedShares from "~/components/indexComponents/themeDetails/RelatedShares";
import BillboardTable from "~/components/indexComponents/themeDetails/BillboardTable";
import MoneyFlow from "~/components/indexComponents/themeDetails/MoneyFlow";
import RelevantNews from "~/components/indexComponents/themeDetails/RelevantNews";
...
export default {
  ...
  head() {
      return {
      title: `${this.themeDetailBasicInfo.conceptStockName} - 主题风向标`,
      //   meta: [
      //     {
      //       hid: "description",
      //       name: "description",
  ...
  async fetch({ store, query }) {
    await Promise.all([
      axios.get(Util.baseUrl() + "conceptDetail?conceptId=" + query.id),
      axios.get(
        Util.baseUrl() +
          "getConceptStockHeroListByDate?type=1&conceptId=" +
          query.id
      )
    ]).then(res => {
      // console.log(res);
      ...
      store.commit("THME_BASIC_INFO", theme.data.conceptStockInfoVo);
      store.commit("THEME_CONCEPT_STOCK", theme.data.sdvList);
      store.commit("THEME_CONCEPT_NEWS", theme.data.infomationList);
  ...
  mounted() {
    let phone =
      sessionStorage.getItem("qutouPhone") ||
      localStorage.getItem("qutouPhone") ||
      null;
