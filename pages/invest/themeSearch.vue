...
<section class="ThemeSearchMore">
    <div class="container">
        <div class="crumbs">
            <span><nuxt-link to="/invest/investCon">主题投资 > </nuxt-link></span>
            <b>主题检索</b>
        </div>
        ...
        <div class="resultArea" v-if="themeList">
            <dl v-for="themeObj in themeList">
                ...
                <dd v-for="theme in themeObj.concepList">
                    <nuxt-link v-if="theme" :to="'/themeDetails?id='+theme.id" target="_blanck">
...
export default {
    ...
    async fetch({store, params}){
        await Promise.all([
            axios.get(Util.baseUrl() + 'selfSelectInfo/showAllInfo')
        ])
        .then(res => {
            let [ list ] = res;
            store.commit('SHOW_ALL_THEME_LIST', list.data);
        });
    },
